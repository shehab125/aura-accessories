...
// Update showSection - wait, this might have worked already
if (!content.includes("loadCustomRequests();")) {
                    ...
                    <tbody>
                        \${requests.map(r => \`
                            <tr>
                                ...
                                <td>
                                    \${r.status === 'pending' || r.status === 'priced' ? \`<input type="number" id="cr-price-\${r.id}" value="\${r.price || ''}" placeholder="السعر" class="form-input" style="width:80px;padding:4px;height:auto;font-size:12px;">\` : \`\${r.price || '-'}\`}
                                </td>
                                <td>
...
content = content.replace(/async function saveOrderChange\(\)\s*\{/, newLogic);
